...
var insertHeaderHandler = function () {
    ...
    if (header_1.supportsLanguage(document.languageId))
        activeTextEditor.edit(function (editor) {
            ...
            if (currentHeader)
                editor.replace(new vscode_1.Range(0, 0, 17, 0), header_1.renderHeader(document.languageId, newHeaderInfo(document, header_1.getHeaderInfo(currentHeader))));
            else
...
var startUpdateOnSaveWatcher = function (subscriptions) {
    return vscode.workspace.onWillSaveTextDocument(function (event) {
        ...
        event.waitUntil(Promise.resolve(header_1.supportsLanguage(document.languageId) && currentHeader ?
            [
                vscode_1.TextEdit.replace(new vscode_1.Range(0, 0, 17, 0), header_1.renderHeader(document.languageId, newHeaderInfo(document, header_1.getHeaderInfo(currentHeader))))
            ]
            : []));
    }, null, subscriptions);
};
